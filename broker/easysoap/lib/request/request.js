(function() {
    ...
    that.soapCall = function(callParams, soapParams, stdParams, opts) {
		...
		//requestParams.headers.push({"content});
        ...
        return new Promise(function(resolve, reject) {
            read(__dirname + path.sep + 'request.tpl', 'utf-8')
                .then(function(tpl) {
					...
					if (config.debug) console.log(_tpl);
                    var data = '';
                    ...
                    var request  = protocol.request(requestParams);
                        ...
                        request.once('response',  function(res) {
							...
							if (config.debug) console.log('STATUS: ' + res.statusCode);
  							if (config.debug) console.log('HEADERS: ' + JSON.stringify(res.headers));
                            ...
                            res.on('end', function() {
								...
								if (config.debug) console.log("REQUEST END");
                                resolve({
                                    'response'  : data,
                                    'header'    : res.headers
                                });
                            ...
                            res.on('data', function(chunk) {
								if (config.debug) console.log("REQUEST DATA");
                                data += chunk;
                            });
						...
						request.setHeader("Content-Length", _tpl.length);
						
                        request.write(_tpl);
						
						if (config.debug) console.log("HEADER INFORMATION");
						if (config.debug) console.log(request);
                        request.end();
                });
        });
    };
